...
public class ServiceAuthForHDServlet extends HttpServlet{
	...
	private static Logger logger = Logger.getLogger(ServiceAuthForHDServlet.class);
	
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		logger.debug(" ServiceAuthForHD begin ");
		...
		if("1".equals(channelId)){
			 SuccessUrl = getRequestPrefix(request)+"/yyzj_hd/filmAction!listAsset.do?filmId="+filmId+"&channelId="+channelId+"&from="+from;
			 FailureUrl = getRequestPrefix(request)+"/yyzj_hd/filmAction!orderConfirm.do?filmId="+filmId;
		}else if("2".equals(channelId)){
			 SuccessUrl = getRequestPrefix(request)+"/crazyenglish_hd/filmAction!listAsset.do?filmId="+filmId+"&channelId="+channelId+"&from="+from;
			 FailureUrl = getRequestPrefix(request)+"/crazyenglish_hd/filmAction!orderConfirm.do?filmId="+filmId;
		}else{
			 SuccessUrl = getRequestPrefix(request)+"/crjy_hd/filmAction!listAsset.do?filmId="+filmId+"&channelId="+channelId+"&from="+from;
