...
function keyEvent() {
	...
	case 33://上一页
		if( curPage > 1){
			...
			var url = "${ctx }/crjy/filmAction!listFilmByColumnId.do?columnId="+columnID+"&curPage="+pageNo+"&pageSize=12";
			location.href = url;
		}
		return 0;
		break;
        ...
        <tr>
          <td align="center" height="27" valign="bottom" class="STYLE14">${pageBean.curPage}/${pageBean.totalPages} 
		           ...
		           <c:when test="${pageBean.curPage > 1}">
		           		<a href="${ctx }/crjy/filmAction!listFilmByColumnId.do?columnId=${requestScope.columnID }&curPage=${pageBean.curPage-1}&pageSize=12" class="STYLE14">上一页</a>
		           </c:when>
		           <c:otherwise>
		           		上一页
		           </c:otherwise>
		           ...
		           <c:when test="${pageBean.curPage < pageBean.totalPages}">
		           		<a href="${ctx }/crjy/filmAction!listFilmByColumnId.do?columnId=${requestScope.columnID }&curPage=${pageBean.curPage+1}&pageSize=12" class="STYLE14">下一页</a>
		           </c:when>
		           <c:otherwise>
		           		下一页
		           </c:otherwise>
