			       ...
			       <c:if test="${fn:length(requestScope.filmList) > 0 }">
				      <c:forEach items="${requestScope.filmList}" begin="0" end="0" var="result" varStatus="status">
				        	<td width="291" height="54" bgcolor="#214c47">
					        	<a href="${ctx }/yyzj/filmAction!detail.do?filmId=${result.id}" id="defaultFocus" onfocus="focusStyle('t_0_${status.count }','titleon');" onblur="focusStyle('t_0_${status.count }','titleoff');">
			  ...
			  <tr bgcolor="#214c47">
	        	  ...
	        	  <c:if test="${fn:length(requestScope.filmList) > 1 }">
		      			<c:forEach items="${requestScope.filmList}" begin="1" end="1" var="result" varStatus="status">
				        	<a href="${ctx }/yyzj/filmAction!detail.do?filmId=${result.id}" onfocus="focusStyle('t_1_${status.count }','titleon');" onblur="focusStyle('t_1_${status.count }','titleoff');">
				        		<div id="t_1_${status.count }" class="titleoff" >${result.name }</div>
				        	</a>
			        	 </c:forEach>
      				</c:if>
	        	</td>
